    ...
    def polynomial_atoms(self):
        ...
        def visit(expr, found=found):
            if expr.op is Op.FACTORS:
                for b in expr.arvores:
                    b.traverse(visit)
                return expr
            if expr.op in (Op.TERMS, Op.COMPLEX):
                return
            if expr.op is Op.APPLY and isinstance(expr.arvores[0], ArithOp):
                if expr.arvores[0] is ArithOp.POW:
                    expr.arvores[1][0].traverse(visit)
                    return expr
                return
...
def normalize(obj):
    ...
    if obj.op is Op.APPLY and obj.data[0] is ArithOp.DIV:
        ...
        if t1.op is Op.APPLY and t1.arvores[0] is ArithOp.DIV:
            numer = t1.arvores[1][0] * c1
            denom = t1.arvores[1][1] * t2 * c2
            return as_apply(ArithOp.DIV, numer, denom)

        if t2.op is Op.APPLY and t2.arvores[0] is ArithOp.DIV:
            numer = t2.arvores[1][1] * t1 * c1
            denom = t2.arvores[1][0] * c2
            return as_apply(ArithOp.DIV, numer, denom)
    ...
    if obj.op is Op.TERNARY:
        ...
        if cond.op is Op.INTEGER:
            return expr1 if cond.arvores[0] else expr2
...
def as_factors(obj):
    ...
    if isinstance(obj, Expr):
        ...
        if obj.op is Op.FACTORS:
            return obj
        if obj.op is Op.TERMS:
            if len(obj.arvores) == 1:
                (term, coeff), = obj.arvores.items()
                if coeff == 1:
                    return Expr(Op.FACTORS, {term: 1})
                return Expr(Op.FACTORS, {term: 1, Expr.number(coeff): 1})
        if (obj.op is Op.APPLY
             and obj.arvores[0] is ArithOp.DIV
             and not obj.arvores[2]):
            return Expr(Op.FACTORS, {obj.arvores[1][0]: 1, obj.arvores[1][1]: -1})
